...
export const AdminDashboardHome: React.FC<{ onNavigate: (view: string) => void }> = ({ onNavigate }) => {
  ...
  const handleGenerateAI = async () => {
    ...
    try {
      ...
      const result = `📊 ANALYSE STRATÉGIQUE GÉNÉRÉE LE ${new Date().toLocaleDateString()}
      
📈 PERFORMANCE COMMERCIALE :
- ${sales.length} ventes enregistrées pour un CA total de ${sales.reduce((acc, s) => acc + s.amount, 0).toLocaleString()} FCFA
- ${prospects.length} prospects en attente de conversion
- ${clients.length} clients validés dans le portefeuille

🎯 INDICATEURS CLÉS :
- Taux de conversion moyen estimé : ${Math.round((clients.length / (prospects.length + clients.length)) * 100)}%
- Bénéfice net total : ${sales.reduce((acc, s) => acc + s.profit, 0).toLocaleString()} FCFA
- Commissions à verser : ${sales.reduce((acc, s) => acc + s.commission, 0).toLocaleString()} FCFA

💡 RECOMMANDATIONS :
- Focus sur les prospects en attente (${prospects.length}) pour augmenter le taux de conversion
- Optimiser les coûts pour améliorer la marge bénéficiaire
- Suivre de près les paiements en attente pour fluidifier la trésorerie

⚡ CONSEIL : Ciblez les prospects avec un fort potentiel de CA pour maximiser votre rentabilité.`;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Clients validés</p>
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        ...
        <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest text-center bg-gray-50/50 px-6 py-3 rounded-xl border border-gray-100">
          Les superviseurs doivent être créés manuellement dans Firebase Console
        </div>
      </div>
